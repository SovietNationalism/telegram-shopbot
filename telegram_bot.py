...
class ShopBot:
    def __init__(self):
        self.products = {
            "1": {"name": "Dry Filtrato 🍫", "price": "3g 30\n5g 40\n10g 70\n15g 100\n25g 160\n35g 215\n50g 250\n100g 420\n200g 780", "description": "Dry con effetto potente e duraturo, e un odore vivace", "video_file_id": "BAACAgQAAxkBAAPCaGv91fParWTOjzMLqrdb2v-pB0wAAksWAAKdJ2FTkI7DxvBinmg2BA"},
            "2": {"name": "Sciroppo al THC 🫗", "price": "x 1 150 ml 30€\nx 2 300 ml 40€\nx 5 750 ml 100€\nx 10 1,5 l 190€\nx 20 3 l 335€", "description": "🇮🇹 SCIROPPO THC\n🍇 Gusti: Lampone, Fragola, Menta, Limone\n\nUna formula esclusiva, realizzata con estratto di hashish a base di etanolo di alta qualità, emulsionato in sciroppo al lampone per una stabilità e biodisponibilità superiore.\n\n💧 Da mescolare con qualsiasi tipo di bevanda! Consigliamo liquidi fredde e dolci per mascherare il sapore.\nOgni bottiglia contiene 250 mg di THC attivo in 150 ml di sciroppo.\n\n📐 Dosaggio consigliato (1.66 mg/ml):\n • 🟢 Principianti: 10–15 mg (6–9 ml circa)\n • 🟡 Regolari: 25–35 mg (15–20 ml)\n • 🔴 Esperti: 50+ mg (30 ml e oltre)", "video_file_id": "BAACAgQAAxkBAAIBCmhsFSRwLTrFoTt3ZbImNDzA8cKtAALKHQACU4xhU_j8jDMDFSiJNgQ"},
            "3_10": {"name": "Caramelle al THC 🇪🇸 - Formato 10", "caption": "📦 *Caramelle al THC 🇪🇸 - Formato 10 caramelle da 500mg*\nRuntz Gummies\nWhite Runtz Fruit Punch - “Ether” Runtz Green Apple - Original Runtz Berries - Pink Runtz Watermelon\n\nSmacker Gummies Sours (Mix Green Apple, Blue Raspberry, Cherry, Lemon & Watermelon)\n\nWarheads Sour Medicated Chewy Cubes\n(Mix Orange, Watermelon, Blue Raspberry, Black Cherry, Strawberry, Green Apple)\n\n1 - 20\n2 - 35\n3 - 50\n4 - 60\n5 - 70\n10 - 130", "video_file_id": "BAACAgQAAxkBAAPvaGwJdobhaO1RPvm1nrbxIKokTOIAAqgdAAJTjGFTqdTZCjgZEpU2BA"},
            "3_20": {"name": "Caramelle al THC 🇪🇸 - Formato 20", "caption": "📦 *Caramelle al THC 🇪🇸 - Formato 20 caramelle 600mg*\nRancher Gummies Original Flavors (Mix Strawberry, Green Apple, Blue Raspberry)\n\nRancher Gummies Sours (Mix Strawberry, Green Apple, Blue Raspberry)\n\nWarheads Sour Medicated Chewy Cubes\n(Mix Orange, Watermelon, Blue Raspberry, Black Cherry, Strawberry, Green Apple)\n\n1 - 25\n2 - 40\n3 - 55\n4 - 70\n5 - 80\n10 - 140", "video_file_id": "BAACAgQAAxkBAAPvaGwJdobhaO1RPvm1nrbxIKokTOIAAqgdAAJTjGFTqdTZCjgZEpU2BA"},
            "4": {"name": "THC Vapes Packwoods™ x Runtz 💨", "price": "1 - 45\n2 - 80\n3 - 110\n4 - 135\n5 - 160\n10 - 300", "description": "1000mg distillato Delta 9 THC", "special_note": "Dettaglio sotto i 5 pz non ancora disponibile. Tra poco.", "video_file_id": "BAACAgQAAxkBAAIBhGhsLZ3WwkXvnqI-G74L_hsWrg6YAAI8GQACw-ZgU0zEehfiSYpmNgQ"}
        }
        self.services = {"1": {"name": "Creazione Bot Telegram", "price": "€35/m", "description": "Sei un venditore o comunque sei interessato alla creazione di un bot simile? Posso aiutarti.", "photo_file_id": "AgACAgQAAxkBAAIBVmY0n9f5v1cAAQ1nUuH4QnX8h3QjAAJ8tzEbJ2FTkJ7yK5y1vN2BAAMCAANzAAMvBA"}}

    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.delete_last_menu(context, update.effective_chat.id)
        kb = [[InlineKeyboardButton("🛍️ Shop 🛍️", callback_data="shop")],[InlineKeyboardButton("💰 Pagamenti 💰", callback_data="payments")],[InlineKeyboardButton("👥 Contattami 👥", callback_data="contact")],[InlineKeyboardButton("👨‍💻 Developer 👨‍💻", callback_data="developer")]]
        msg = "🎉 Benvenuto sul bot Vetrina ItalianEdibles! 🇮🇹\n\nScopri un mondo di prodotti selezionati, pensati per un'esperienza unica e indimenticabile. Puoi esplorare, acquistare e contattarci in pochi semplici clic!"
        m = update.effective_message
        try:
            sent = await m.reply_photo(photo=WELCOME_IMAGE_URL, caption=msg, reply_markup=InlineKeyboardMarkup(kb))
            context.user_data["last_menu_msg_id"] = sent.message_id
        except BadRequest: sent = await m.reply_text(text=msg, reply_markup=InlineKeyboardMarkup(kb)); context.user_data["last_menu_msg_id"] = sent.message_id
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        q = update.callback_query; await q.answer(); d = q.data; cid = q.message.chat.id
        await self.delete_last_menu(context, cid)
        if d == "back_to_main": await self.start(update, context); return
        if d == "shop":
            kb = [[InlineKeyboardButton("📱 Prodotti", callback_data="products")],[InlineKeyboardButton("🔧 Servizi", callback_data="services")],[InlineKeyboardButton("⬅️ Indietro", callback_data="back_to_main")]]
            ...
            context.user_data["last_menu_msg_id"] = sent.message_id; return
        if d == "payments":
            kb = [[InlineKeyboardButton("⬅️ Indietro", callback_data="back_to_main")]]
            sent = await context.bot.send_message(chat_id=cid, text="💰 *METODI DI PAGAMENTO*\n\n• 💰 Crypto (0% commissione)\n• 💵 Contanti (0% commissione)\n• 🏦 Bonifico istantaneo (0% commissione)\n• 💳 PayPal (+10% commissione)\n\nScegli il metodo che preferisci al momento dell'ordine.", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
            context.user_data["last_menu_msg_id"] = sent.message_id; return
        if d == "contact":
            kb = [[InlineKeyboardButton("✉️ Contattami su Telegram", url="https://t.me/ItalianEdibles")],[InlineKeyboardButton("⬅️ Indietro", callback_data="back_to_main")]]
        ...
        if d in ["products","back_to_products"]:
            kb = [[InlineKeyboardButton(self.products["1"]["name"], callback_data="product_1")],[InlineKeyboardButton(self.products["2"]["name"], callback_data="product_2")],[InlineKeyboardButton("Caramelle THC 🇪🇸 - Formato 10", callback_data="product_3_10"),InlineKeyboardButton("Caramelle THC 🇪🇸 - Formato 20", callback_data="product_3_20")],[InlineKeyboardButton(self.products["4"]["name"], callback_data="product_4")],[InlineKeyboardButton("⬅️ Indietro", callback_data="shop")]]
        ...
        if d.startswith("product_3_"):
            k = d.replace("product_",""); p = self.products.get(k)
            if not p: await q.answer("❌ Prodotto non trovato!"); return
            try:
        ...
        if d in ["product_1","product_2"]:
            pid = d.split("_")[1]; p = self.products.get(pid)
            if not p: await q.answer("❌ Prodotto non trovato!"); return
        ...
        if d.startswith("service_"):
            sid = d.split("_")[1]; s = self.services.get(sid)
            if not s: await q.answer("❌ Servizio non trovato!"); return
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if m.from_user and m.from_user.id == ADMIN_USER_ID:
            if m.video: await m.reply_text(f"File ID del video:\n<code>{m.video.file_id}</code>", parse_mode=ParseMode.HTML); return
            if m.photo: await m.reply_text(f"File ID della foto:\n<code>{m.photo[-1].file_id}</code>", parse_mode=ParseMode.HTML); return
        t = m.text.lower() if m.text else ""
        if any(w in t for w in ["ciao","salve"]): await m.reply_text("Ciao! 👋 Usa /start per iniziare.")
        elif "aiuto" in t or "help" in t: await m.reply_text("Usa /start per vedere il menu principale.")
        else: await m.reply_text("Non ho capito. Usa /start per vedere le opzioni disponibili.")
